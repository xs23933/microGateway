...
type Upstream interface {
	...
	AddHost(host, name string) error
	DelHost(host string) int

	// 添加可用性检测
	SetHealthCheck(path, method string, interval, timeout time.Duration)
...
func (uh *UpstreamHost) Info() map[string]interface{} {
	return map[string]interface{}{
		"name":      uh.Server,
		"host":      uh.Name,
		"available": uh.Available(),
	}
}
...
func (p Proxy) ServeHTTP(w http.ResponseWriter, r *http.Request) (int, error) {
	...
	upstream := p.match(r)
	if upstream == nil {
		return http.StatusNotFound, fmt.Errorf("not match") // p.Next.ServeHTTP(w, r)
	}
...
func (p Proxy) match(r *http.Request) Upstream {
	...
	p.APIs.Range(func(key, value interface{}) bool {
		k, ok := key.(string)
		if !ok {
			return true
		}

		hasGeneric := strings.HasSuffix(k, "*")
		if hasGeneric {
