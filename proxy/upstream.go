...
func NewUpstream(host, name string) (Upstream, error) {
	...
	for i, host := range to {
		uh, err := upstream.NewHost(host, name)
		if err != nil {
			return nil, err
		}
		upstream.Hosts[i] = uh
...
func NewStaticUpstreams(c caddyfile.Dispenser, host string) ([]Upstream, error) {
	...
	for c.Next() {
		...
		upstream.Hosts = make([]*UpstreamHost, len(to))
		for i, host := range to {
			uh, err := upstream.NewHost(host, host)
			if err != nil {
				return upstreams, err
			}
			upstream.Hosts[i] = uh
...
func (upstream *staticUpstream) GetHealthCheck() HealthCheck {
	return HealthCheck{
		Method:   upstream.HealthCheck.Method,
		Path:     upstream.HealthCheck.Path,
		Interval: int(upstream.HealthCheck.Interval / time.Second),
		Timeout:  int(upstream.HealthCheck.Timeout / time.Second),
	}
}
func (upstream *staticUpstream) SetHealthCheck(path, method string, interval, timeout time.Duration) {
	if upstream.HealthCheck.Path != "" { // haved HealthCheck
		return
	}
	upstream.HealthCheck.Interval = interval
	upstream.HealthCheck.Timeout = timeout
	upstream.HealthCheck.Path = path
	upstream.HealthCheck.Method = method
	...
	if upstream.from != "" {
		hostHeader := upstream.upstreamHeaders.Get("Host")
		if strings.Contains(hostHeader, "{host}") {
			upstream.HealthCheck.Host = strings.Replace(hostHeader, "{host}", upstream.from, -1)
		}
	}
	upstream.wg.Add(1)
	go func() {
...
func (u *staticUpstream) AddHost(host, name string) error {
	h, err := u.NewHost(host, name)
	if err != nil {
		log.Panicln("new host failed", host)
		return err
	}
...
func (u *staticUpstream) DelHost(host string) int {
	for idx, h := range u.Hosts {
		if h.Name == host {
			u.Hosts = append(u.Hosts[:idx], u.Hosts[idx+1:]...)
			return u.GetHostCount()
		}
	}
	return u.GetHostCount()
}

func (u *staticUpstream) NewHost(host, name string) (*UpstreamHost, error) {
	...
	uh := &UpstreamHost{
		...
		CheckDown: func(u *staticUpstream) UpstreamHostDownFunc {
			return func(uh *UpstreamHost) bool {
				// log.Printf("check healthy %d fails %d\n", atomic.LoadInt32(&uh.Unhealthy), atomic.LoadInt32(&uh.Fails))
	...
	for _, host := range u.Hosts {
		...
		for _, addr := range candidates {
			...
			unhealthy := func() bool {
				...
				req, err := http.NewRequest(u.HealthCheck.Method, hostURL, nil)
				if err != nil {
					return true
				}
...
func (u *staticUpstream) GetHosts() HostPool {
	return u.Hosts
}
